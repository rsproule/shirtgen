      ...
      mutations.forEach(mutation => {
        mutation.addedNodes.forEach(node => {
          if (node.nodeType === Node.ELEMENT_NODE) {
            ...
            const errorElement =
              (element.querySelector && element.querySelector(".echo-error")) ||
              (element.querySelector &&
                element.querySelector('[class*="error"]')) ||
              (element.classList && element.classList.contains("echo-error")
                ? element
                : null);
            ...
            if (errorElement) {
              ...
              if (
                errorText?.includes("Session renewal failed") ||
